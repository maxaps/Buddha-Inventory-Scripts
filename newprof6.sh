# This section will make a new .txt file with the system serial number as the name with a pattern-recognizing grep command that IDs the system serial number and returns it as a variable
# The variable will be inserted in the mkfile target.

cd ~/Desktop
mkfile -n b Profile.txt
echo GENERAL SYSTEM >> Profile.txt
system_profiler SPHardwareDataType | egrep -w "Number|Model|Processor|Total" >> Profile.txt
...
system_profiler SPStorageDataType | egrep -w "File|Capacity|Medium" >> Profile.txt
